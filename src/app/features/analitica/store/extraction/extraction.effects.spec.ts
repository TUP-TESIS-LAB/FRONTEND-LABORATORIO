...
describe('ExtractionEffects', () => {
  ...
  function setup(branchId: number | null = 7): void {
    ...
    TestBed.configureTestingModule({
      providers: [
        ...
        provideMockStore({
          initialState: {
            [EXTRACTION_FEATURE_KEY]: { ...initialExtractionState, selectedBranchId: branchId },
          },
        }),
  ...
  describe('with a selected branch', () => {
    ...
    it('loadMine$ uses branchId from the store', async () => {
    ...
    it('refreshAll$ dispatches the 4 loads', async () => {
    ...
    it('setSelectedBranch triggers refreshAll', async () => {
      const promise = firstValueFrom(effects.branchChangeRefresh$.pipe(take(1)));
      actions$.next(A.setSelectedBranch({ branchId: 9 }));
      expect(await promise).toEqual(A.refreshAll());
    });
    ...
    it('assignExtractor$ Success triggers a refreshAll via mutationRefresh$', async () => {
      ...
      actions$.next(A.assignExtractor({ id: 10, box: 2, branchId: 7 }));
