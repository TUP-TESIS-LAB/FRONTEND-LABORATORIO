...
export const queueReducer = createReducer(
  initialQueueState,
  on(A.loadQueue, (s, { silent }) => silent ? s : { ...s, loading: true, error: null }),
  ...
  // Cancel no toca callingId — son operaciones distintas.
  on(A.cancelQueueEntryFailure, (s, { error }) => ({ ...s, error })),
);
